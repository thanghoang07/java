...
public class Cohen_sutherland extends JPanel {
	...
	boolean trivial_validations(Dimension p1, Dimension p2) {
		if (p1.width < left && p2.width < left || p1.width > right && p2.width > right
				|| p1.height < top && p2.height < top || p1.height > down && p2.height > down)
			return false;
		return true;
	}
	...
	boolean not_is_inside(Dimension p) {
		if (p.width < left || p.width > right || p.height < top || p.height > down)
			return true;
		else
			return false;
	}
	...
	int find_position(Dimension p) {
		if (p.width < left)
			return 1;
		else if (p.width > right)
			return 2;
		else if (p.height < top)
			return 4;
		else if (p.height > down)
			return 8;
		return 0;
	}

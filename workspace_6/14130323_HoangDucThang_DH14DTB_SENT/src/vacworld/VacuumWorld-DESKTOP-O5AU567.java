...
public class VacuumWorld extends Environment {
	...
	protected static final int MAX_ACTIONS = 1000;
	...
	public void start(State initState) {
		...
		while (!isComplete() && !quit) {
			p = getPercept(a);
			a.see(p); // nhìn 
			action = a.selectAction(); // hành động
			updateState(a, action); // cập nhật lại trạng thái
			((VacuumState) state).printMatrix(((VacuumState) state).getMap(), output); // in
																						// ma
																						// tran
																						// 1
			//((VacuumState) state).printMatrixPath(((VacuumState) state).getMapHome(), output); // in
																								// ma
																								// tran
																								// 2
			if (interactive == true)
	...
	public static void main(String[] args) {
		int width, height, move;
		sc = new Scanner(System.in);
		VacuumWorld world;
		Agent agent = new VacAgent();
		VacuumState initState = null;
		...
		height = sc.nextInt();		
		System.out.println("＝＝＝ありがとう　ごさいます＝＝＝");
		System.out.printf("Chiều dài %d chiều rộng %d \n", width, height);
		System.out.println("The Vacuum Cleaner World Agent Testbed test 4");
		System.out.println("--------------------------------------");
		//
		if (width <= 5 && height <= 5 || height <= 5 || width <= 5) {
			System.out.println("Loi :D, Nhap so > 5 :D ");
		} else {
			world = new VacuumWorld(interactive);
			//
			initState = VacuumState.getInitState(width, height);
			//
			...
			//
			System.out.println();
			System.out.printf("Size path: %d \n", initState.pathPoint.size());
			initState.printPath();
			System.out.println("Phan quay ve diem bat dau chua lam xong!");
			//
			if (world.timedOut()) {
	...
	public boolean gotHome() {
		...
		if (((VacuumState) state).isRobotOff() && vstate.getAgentX() == 3 && vstate.getAgentY() == 3)
			return true;
		else
			return false;
	}

...
public abstract class Bai2_1<E extends Double> {
	...
	public E[][] congHaiMaTran(E[][] matran1, E[][] matran2) {
		...
		E[][] ketQua1 = (E[][]) new Double[matran1.length][matran1[0].length];
	...
	// nhan ma tran
	public E[][] nhanHaiMaTran2_1(E[][] matran1, E[][] matran2) {
		if (matran1[0].length != matran2.length)
			;

		E[][] ketQua2 = (E[][]) new Double[matran1.length][matran2[0].length];

		for (int i = 0; i < ketQua2.length; i++) {
			for (int j = 0; j < ketQua2[0].length; j++) {
				ketQua2[i][j] = zero();
				for (int k = 0; k < matran1[0].length; k++) {
					ketQua2[i][j] = congLai(ketQua2[i][j], nhanVao(matran1[i][k], matran2[k][j]));
				}
			}
		}
		return ketQua2;
	...
	public static void inRaKetQua(Double[][] matran1, Double[][] matran2, Double[][] matran3, char op) {

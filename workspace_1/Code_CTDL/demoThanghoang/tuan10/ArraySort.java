...
public class ArraySort<T extends Comparable<T>> {
	...
	public int demSoSanhQuickSort = 0, demHoanViQuickSort = 0, demHoanViBubbleSort = 0, demSoSanhBubbleSort = 0,
			demHoanViInsertionSort = 0, demSoSanhInsertionSort = 0;

	public ArraySort(int max) {
		// constructor
		contents = (T[]) new Comparable[max]; // create the array
		num = 0; // no items yet
	}

	// put element into array
	public void intsert(T value) {
		contents[num] = value; // insert it
		num++; // increment size
	}
	...
	public void bubbleSort() {
		...
		for (int i = num - 1; i > 1; i--) {
			for (int j = 0; j < i; j++) {
				if (contents[j].compareTo(contents[j + 1]) > 0) {
					tmp = contents[j];
	...
	public void bubbleSortNew() {
		...
		while (swapped) {
			...
			for (int i = 0; i < num - 1; i++) {
				if (contents[i].compareTo(contents[i + 1]) > 0) {
					tmp = contents[i];
	...
	public void selectionSort() {
		...
		for (int i = 0; i < num - 1; i++) {
			...
			for (int j = i + 1; j < num; j++) {
				if (contents[j].compareTo(contents[minIndex]) < 0) {
					minIndex = j;
				}
			}
			if (minIndex != i) {
				T tmp = contents[i];
				contents[i] = contents[minIndex];
				contents[minIndex] = tmp;
			}
		}
	}
	...
	public void insertionSort() {
		...
		for (int i = 1; i < num; i++) {
			...
			while (j > 0 && contents[j - 1].compareTo(temp) > 0) {// until one is
																// smaller
				contents[j] = contents[j - 1]; // shift item right,
	...
	private int findParrtition(int left, int right) {
		int i = left, j = right;
		T pivotElement = contents[(left + right) / 2];
		while (i <= j) {
			while (contents[i].compareTo(pivotElement) < 0) {
				i++;
				demSoSanhQuickSort++;
			}
			while (contents[j].compareTo(pivotElement) > 0) {
				j--;
				demSoSanhQuickSort++;
			}
			if (i <= j) {
			...
			if (right <= max - min)
				if (left <= pivot - min)
					if (tmp[left].compareTo(tmp[right]) > 0)
						contents[i + min] = tmp[right++];
					else
						contents[i + min] = tmp[left++];
				else
					contents[i + min] = tmp[right++];
			else
				contents[i + min] = tmp[left++];

...
class ATMPanel extends Panel {
	 ...
	 *            the the overall GUI
	 * @param operatorPanel
	 *            the simulation of the card reader
	 * @param cardReader
	 *            the simulation of the card reader
	 * @param display
	 *            the simulation of the display
	 * @param keyboard
	 *            the simulation of the keyboard
	 * @param cashDispenser
	 *            the simulation of the cash dispenser
	 * @param receiptPrinter
	 *            the simulation of the receipt printer
	 */
	ATMPanel(final GUI gui, SimOperatorPanel operatorPanel, SimCardReader cardReader, SimDisplay display,
		...
		atmLayout.setConstraints(operatorPanel,
				GUI.makeConstraints(OPERATOR_ROW, OPERATOR_COL, OPERATOR_WIDTH, OPERATOR_HEIGHT, OPERATOR_FILL));
		...
		atmLayout.setConstraints(cardReaderPanel,
				GUI.makeConstraints(READER_ROW, READER_COL, READER_WIDTH, READER_HEIGHT, READER_FILL));

		// The customer console consists of two components (display and
		// keyboard)
		// that must be added individually in different places in the GUI

		add(display);
		atmLayout.setConstraints(display,
				GUI.makeConstraints(DISPLAY_ROW, DISPLAY_COL, DISPLAY_WIDTH, DISPLAY_HEIGHT, DISPLAY_FILL));

		add(keyboard);
		atmLayout.setConstraints(keyboard,
				GUI.makeConstraints(KEYBOARD_ROW, KEYBOARD_COL, KEYBOARD_WIDTH, KEYBOARD_HEIGHT, KEYBOARD_FILL));

		// Add the cash dispenser

		add(cashDispenser);
		atmLayout.setConstraints(cashDispenser,
				GUI.makeConstraints(DISPENSER_ROW, DISPENSER_COL, DISPENSER_WIDTH, DISPENSER_HEIGHT, DISPENSER_FILL));

		// Put envelope acceptor in a Panel with GridLayout to ensure it gets
		// space
		// even when invisible, then add it
		...
		atmLayout.setConstraints(envelopeAcceptorPanel,
				GUI.makeConstraints(ENVELOPE_ROW, ENVELOPE_COL, ENVELOPE_WIDTH, ENVELOPE_HEIGHT, ENVELOPE_FILL));
		...
		atmLayout.setConstraints(receiptPrinter,
				GUI.makeConstraints(PRINTER_ROW, PRINTER_COL, PRINTER_WIDTH, PRINTER_HEIGHT, PRINTER_FILL));
		...
		atmLayout.setConstraints(showLogButtonPanel, GUI.makeConstraints(SHOW_LOG_BUTTON_ROW, SHOW_LOG_BUTTON_COL,
				SHOW_LOG_BUTTON_WIDTH, SHOW_LOG_BUTTON_HEIGHT, SHOW_LOG_BUTTON_FILL));
	}

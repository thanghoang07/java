...
public class SlidePuzzle extends Applet implements Runnable {
	...
	public void run() {
		if (!m_fAllLoaded) {
			...
			logo = getImage(getDocumentBase(), "D:\\OneDrive\\Documents\\workspace_1\\Thu\\swing\\khac\\icesid.jpg");
		...
		while (true) {
			for (x = 1; x < 9; x++) {
				...
				m_Graphics.copyArea(squares[order[x]].x, squares[order[x]].y, 60, 60,
						squares[order[x - 1]].x - squares[order[x]].x, squares[order[x - 1]].y - squares[order[x]].y);

...
public class BallPanle extends JFrame {
	private InBalls ball = new InBalls();
	private JButton jbtSuspend = new JButton("Suspend");
	private JButton jbtResume = new JButton("Resume");

	public BallPanle() {
		JPanel panel = new JPanel(); // Group buttons in a panel
...
class AddBall extends JPanel {
	public AddBall() {
		setLayout(null);
		add(new InBalls("red", 10 - (int) Math.round((Math.random() * 20)),
				10 - (int) Math.round((Math.random() * 20))));
	}
}

@SuppressWarnings("serial")
class InBalls extends JPanel implements Runnable {
	...
	public InBalls(String ballColor, int vx, int vy) {
		if (ballColor == "red") {
			color = Color.RED;

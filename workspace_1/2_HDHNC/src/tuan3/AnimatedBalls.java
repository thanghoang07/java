...
public class AnimatedBalls {
	...
	public class Balls extends JPanel {
		...
		public Balls() {
			...
			add(new Ball("red", 10 - (int) Math.round((Math.random() * 10)),
					10 - (int) Math.round((Math.random() * 10))));
			add(new Ball("blue", 10 - (int) Math.round((Math.random() * 10)),
					10 - (int) Math.round((Math.random() * 10))));
		}
	}
	...
	public class Ball extends JPanel implements Runnable {
		...
		protected void paintComponent(Graphics g) {
			...
			g.setColor(color);
			g.fillOval(0, 0, 10, 10); // adds color to circle

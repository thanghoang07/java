...
public class UndirectedGraph extends GraphTheory {
	...
	public void inDuongDiD(int[] pred, int src, int des) {
	...
	public void algoDijkstra2(int src) {
		int[] parent = new int[numVex];
		int[] dis = new int[numVex];

		Comparator<Integer> shortestDistance = new Comparator<Integer>() {
		...
		PriorityQueue<Integer> queue = new PriorityQueue<Integer>(10, shortestDistance);
	...
	public void algoDijkstra3(int src, int des) {
		int XP, min;
		for (int i = 0; i < numVex; i++) {
			dist[i] = Integer.MAX_VALUE;
			pred[i] = src;
		...
		while (XP != des) {
			for (int j = 0; j < numVex; j++)
				if (matrix[XP][j] > 0 && dist[j] > matrix[XP][j] + dist[XP] && !visited[j]) {
					dist[j] = matrix[XP][j] + dist[XP];
					pred[j] = XP;
				}

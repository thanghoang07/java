...
public class Bai5 {
	...
	public static int[][] truMatrix(int[][] matrix1, int[][] matrix2, int soHang) {
		int[][] matrixPlus = new int[soHang][soHang];
		for (int i = 0; i < matrixPlus.length; i++) {
			for (int j = 0; j < matrixPlus[0].length; j++) {
				matrixPlus[i][j] = matrix1[i][j] - matrix2[i][j];
	...
	public static void main(String[] args) {
		...
		int[][] matrix3 = congMatrix(matrix1, matrix2, soHang);
		System.out.println("In ra ket qua: ");
		inRaKetQua(matrix1, matrix2, matrix3, '+');
		System.out.println("tru hai matrix");
		int[][] matrix4 = truMatrix(matrix1, matrix2, soHang);
		System.out.println("In ra ket qua: ");
		inRaKetQua(matrix1, matrix2, matrix4, '-');
	}
}
